...
deletefile(b+".html",function(d){update_index(function(e){if(d){return show_error("err_list",d);}if(e){return show_error("err_list",e);}$("#paper_"+a).remove();
});});}$("#dialog_preview").on("show.bs.modal",function(){preview.html(marked(editor.val()));});$("#dialog_publish").on("show.bs.modal",function(){$("#paper_datetime").val(moment(new Date()).format("YYYY-MM-DD HH:mm:ss"));
$("#err_publish").html("");$("#paper_mode").removeAttr("checked");});function append_list(b,a,d){$("#paper_list").append('<li class="list-group-item" id="paper_'+b+'"><a href="/'+d+'.html" target="_blank">'+a+'</a><p class="pull-right"><a href="javascript:edit_paper(\''+d+'\')" title="edit">E</a> | <a href="javascript:delete_paper('+b+", '"+d+'\')" title="delete">D</a></p></li>');
}$("#dialog_list").on("show.bs.modal",function(){$("#paper_list").empty();$("#paper_list_search").val("");paper_list=[];readdir(function(a,b){if(a){return show_error("err_list",a);

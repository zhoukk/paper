...
var f=function(){$(this).parent().removeClass("has-error");};$("#username").focus(f);$("#password").focus(f);$("#paper_title").focus(f);$("#datetime_picker").datetimepicker({format:"YYYY-MM-DD HH:mm:ss",showTodayButton:true,showClear:true,});
$("#upload_file").fileinput({language:"zh",showUpload:false,});$("#dialog_login").modal({backdrop:"static",show:true,keyboard:false});var c=document.cookie;
if(c&&c!=""){$("#username").val(c.split("=")[1].split(";")[0]);}function show_error(b,a){console.log(a);$("#"+b).html('<div class="alert alert-danger" role="alert">'+a.error+" "+a.request.statusText+"</div>");
...
$("#paper_mode").removeAttr("checked");});$("#dialog_upload").on("show.bs.modal",function(){$("#upload_name").val("");});$("#dialog_setting").on("show.bs.modal",function(){$("#err_setting").empty();
});function append_list(b,a,d){$("#paper_list").append('<li class="list-group-item" id="paper_'+b+'"><a href="http://'+login_name+".github.io/"+d+'.html" target="_blank">'+a+'</a><p class="pull-right"><a href="javascript:edit_paper(\''+d+'\')" title="edit">E</a> | <a href="javascript:delete_paper('+b+", '"+d+'\')" title="delete">D</a></p></li>');
...
if(a){return show_error("err_setting",a);}$("#dialog_setting").modal("hide");});});$("#upload_file").on("fileloaded",function(h,g,b,e,a){var j=new Date();
var i="/"+j.getFullYear()+"/"+(j.getMonth()+1);console.log(h,g,b,e,a);});$("#btn_upload").click(function(){});$("#btn_publish").click(function(){var e={};
e.title=html_encode($("#paper_title").val());e.keywords=html_encode($("#paper_keywords").val());e.description=html_encode($("#paper_description").val());

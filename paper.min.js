...
b(d,e);});}function writefile(b,g,e,d){var a={author:{name:login_name,email:login_email},committer:{name:login_name,email:login_email},encode:true};if(!repo_idle){return;
...
$("#upload_file").fileinput("reset");$("#btn_upload").attr("disabled","disabled");});$("#dialog_setting").on("show.bs.modal",function(){$("#err_setting").empty();
...
paper_host="http://"+login_name+".github.io";document.cookie="__paper_user="+h;repo=g.getRepo(login_name,login_name+".github.io");repo.show(function(l,k){if(l){a.createRepo({name:login_name+".github.io"},function(n,m){if(n!=null){$("#btn_login").removeAttr("disabled");
...
n=false;}e(n);});}});});});function create_rss_atom(h,b){var e=new Date();var a='<?xml version="1.0" encoding="UTF-8"?>';a+='<feed xmlns="http://www.w3.org/2005/Atom">';
...
if(a){return show_error("err_setting",a);}$("#dialog_setting").modal("hide");});});var upload_data;$("#upload_file").on("fileloaded",function(i,g,b,e,a){var k=new Date();
var j=k.getFullYear()+"/"+(k.getMonth()+1)+"/";$("#upload_prefix").val(j+$(this).val());if(typeof a.result==="string"){upload_data=a.result;$("#btn_upload").removeAttr("disabled");
}else{var h=new FileReader();h.onloadend=function(){upload_data=h.result;$("#btn_upload").removeAttr("disabled");};h.readAsBinaryString(new Blob([a.result]));
}});$("#upload_file").on("fileclear",function(){var a=new Date();$("#upload_prefix").val(a.getFullYear()+"/"+(a.getMonth()+1)+"/");$("#err_upload").empty();
$("#btn_upload").attr("disabled","disabled");});$("#upload_file").on("filebatchselected",function(){$("#err_upload").empty();});$("#btn_upload").click(function(){$("#err_upload").empty();
$(this).attr("disabled","disabled");var a=$("#upload_prefix").val();writefile(a,upload_data,function(b){$("#btn_upload").removeAttr("disabled");if(b){return show_error("err_upload",b);
}var d=paper_host+"/"+a;$("#err_upload").html(d);});});$("#btn_publish").click(function(){var e={};e.title=html_encode($("#paper_title").val());e.keywords=html_encode($("#paper_keywords").val());
e.description=html_encode($("#paper_description").val());e.datetime=$("#paper_datetime").val();e.md=editor.val();if($("#paper_mode").is(":checked")){e.mode="draft";
}else{e.mode="publish";}if(e.title==""){$("#paper_title").parent().addClass("has-error");return;}var a=marked(e.md);var g=moment(e.datetime).toDate();var b=g.getFullYear()+"/"+(g.getMonth()+1);
e.path=b+"/"+e.title;$(this).attr("disabled","disabled");$.get("paper.tpl",function(h){h=h.replace(/{{body}}/,a).replace(/{{title}}/,e.title).replace(/{{author}}/,config.author).replace(/{{disqus}}/,config.disqus).replace(/{{keywords}}/,e.keywords).replace(/{{description}}/,e.description).replace(/{{datetime}}/,e.datetime);
